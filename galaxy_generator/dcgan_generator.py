...
class DCGAN_Generator(BaseTrainer):
    ...
    def __init__(self, config):
        ...
        self.fixed_noise = torch.randn(
            self.batch_size, self.n_zlatent, 1, 1, device=self.device)
    ...
    def _build_model(self, config):
        ...
        print('Generator  :', sum(param.numel()
              for param in self.netG.parameters() if param.requires_grad))
        print('Discriminator  :', sum(param.numel()
              for param in self.netG.parameters() if param.requires_grad))
    ...
    def _init_storage(self):
        ...
        torch.save(self.fixed_noise, os.path.join(
            self.dir_genImgs, 'fixed_noise.pt'))
    ...
    def _train_one_epoch(self):
        ...
        for id_batch, x in enumerate(self.dataloader):
            ...
            if self.current_iteration % self.freq_img == 0:
                ...
                self._make_png(
                    generator_output=gen_out, iterID=self.current_iteration, epochID=self.current_epoch)
    ...
    def _make_png(self, generator_output, iterID, epochID):
        ...
        plt.imshow(np.transpose(vutils.make_grid(generator_output,
                   padding=2, normalize=True, nrow=self.nrow), (1, 2, 0)))
        plt.tight_layout()
        file_img = os.path.join(self.dir_genImgs, f'{iterID}_ep{epochID}.png')
        fig.savefig(file_img, dpi=self.dpi,
